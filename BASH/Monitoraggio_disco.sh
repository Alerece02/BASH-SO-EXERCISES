...
df -P | grep -vE '^Filesystem|tmpfs|cdrom' | awk '{ print $5 " " $1 }' | while read output; do
    ...
    if [ $usep -ge $THRESHOLD ]; then
        echo "Attenzione: lo spazio su disco nella partizione $partition ha raggiunto il $usep%." | mail -s "Avviso spazio su disco" cahiviv328@inpsur.com
    fi
done
